...
import {
  LayoutDashboard, FileText, Files, FolderTree, Image as ImageIcon,
  ListTree, Users, LogOut, ExternalLink, Newspaper, SlidersHorizontal, ShieldCheck, UsersRound,
} from "lucide-react";
...
export function Badge({ tone, children }: { tone: "draft" | "published" | "muted"; children: ReactNode }) {
  ...
  return (
    <span className={`inline-block rounded-full border px-2 py-0.5 text-[0.72rem] font-bold uppercase tracking-wide ${cls}`}>
      {children}
    </span>
  );
}
...
export type AdminUserLite = { displayName: string; username: string; role: string; permissions?: Permission[] };

type NavItem = { to: string; label: string; Icon: ComponentType<{ size?: number }>; group: string; perm?: Permission };
const NAV: NavItem[] = [
  ...
  { group: "Content", to: "/admin/categories", label: "Categories", Icon: FolderTree, perm: "categories.manage" },
  ...
  { group: "Content", to: "/admin/fields", label: "Custom Fields", Icon: SlidersHorizontal, perm: "fields.manage" },
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  const initials = (user?.displayName ?? "Admin").trim().split(/\s+/).map((s) => s[0]).slice(0, 2).join("").toUpperCase();
  return (
    <div className="admin-shell">
      <aside className="admin-sidebar">
        <Link to="/admin" className="admin-brand">
          <Newspaper size={20} className="text-[var(--admin-accent)]" /> Bract<span className="dot">·</span>CMS
        </Link>
        ...
        <div className="admin-user">
          ...
          <Form method="post" action="/admin/logout" className="m-0" style={{ marginLeft: "auto" }}>
            <button type="submit" className="icon-btn" aria-label="Log out"><LogOut size={16} /></button>
          </Form>
        </div>
      </aside>
      <div className="admin-content">
        <header className="admin-topbar">
          ...
          <Link to="/" className="icon-btn" aria-label="View site" title="View site"><ExternalLink size={16} /></Link>
        </header>
    ...
    <nav aria-label="Breadcrumb" className="mb-2 text-sm text-slate-500">
      {items.map((it, i) => (
        <span key={i}>
          ...
          {it.to ? (
            <Link to={it.to} className="text-[var(--accent)] no-underline">{it.label}</Link>
          ) : (
            <span>{it.label}</span>
          )}
        </span>
...
function MenuList({ items, ulClass, submenuClass, itemClass }: { items: MenuNode[]; ulClass: string; submenuClass: string; itemClass: string }) {
    ...
    <ul className={ulClass || undefined}>
      {items.map((n) => (
        <li key={n.id} className={[itemClass, n.cssClass].filter(Boolean).join(" ") || undefined}>
          ...
          {n.children.length > 0 ? <MenuList items={n.children} ulClass={submenuClass} submenuClass={submenuClass} itemClass={itemClass} /> : null}
        </li>
      ))}
    </ul>
  );
...
export function MenuNav({ menu, fallbackClass = "site-nav", ariaLabel }: { menu?: ResolvedMenu; fallbackClass?: string; ariaLabel?: string }) {
  if (!menu || menu.items.length === 0) return null;
  return (
    <nav aria-label={ariaLabel}>
      <MenuList items={menu.items} ulClass={menu.menuClass || fallbackClass} submenuClass={menu.submenuClass} itemClass={menu.itemClass} />
    </nav>
  );
}

export function SiteFrame({ header, footer, children }: { header?: ResolvedMenu; footer?: ResolvedMenu; children: ReactNode }) {
  return (
    <div className="site">
      <header className="masthead">
        <Link to="/" className="title">The Bract Gazette</Link>
      ...
      <footer className="site-footer">
        ...
        <div>A BractJS example · admin at <Link to="/admin">/admin</Link></div>
      </footer>
    </div>
  );
}
    ...
    <article className="grid gap-2 border-b border-[var(--line)] pb-6">
      ...
      <div className="flex items-center gap-2.5 text-xs text-slate-500">
        ...
        {post.category ? (
          <>
            <span>·</span>
            <Link to={`/category/${post.category.slug}`} className="text-[var(--accent)] no-underline [font-variant:small-caps]">
              {post.category.name}
            </Link>
          </>
        ) : null}
      ...
      <h2 className="m-0 font-[var(--display)] text-[1.7rem] leading-tight">
        <Link to={`/posts/${post.slug}`} className="no-underline">{post.title}</Link>
      </h2>
  ...
  return (
    <section style={{ marginTop: "2rem", display: "grid", gap: "1.1rem" }}>
      {fields.map(({ field, values }) => (
        <div key={field.id}>
          <h3 className="prose" style={{ fontSize: "1.05rem", margin: "0 0 .4rem" }}>{field.label}</h3>
          <div style={{ display: "flex", flexWrap: "wrap", gap: ".6rem", alignItems: "center" }}>
            {values.map((v, i) => v.type === "text" ? (
              <span key={i}>{v.text}</span>
            ) : v.type === "image" ? (
              <Image key={i} src={v.url} alt={v.alt} width={240} height={240} style={{ width: 120, height: 120, objectFit: "cover", borderRadius: "8px" }} />
            ) : v.url ? (
              <Link key={i} to={v.url} style={{ color: "var(--accent)", fontWeight: 600 }}>{v.title}</Link>
            ) : (
