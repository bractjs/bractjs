...
export async function loader({ request, params }: LoaderArgs): Promise<Data> {
  ...
  return {
    cat,
    options: categoryTreeFlat(),
    blocked: [...categoryDescendantIds(cat.id)],
    customFields: loadEntityFields("category", cat.id),
  };
}
...
export function ErrorBoundary({ error }: { error: unknown }) {
  ...
  return (
    <div className="admin-panel">
      ...
      <Link to="/admin/categories" style={{ color: "var(--accent)", fontWeight: 600 }}>
        ← Back to categories
      </Link>
    </div>
  );
  ...
  return (
    <>
      <div className="admin-bar">
        ...
        <Link to="/admin/categories" style={{ color: "var(--accent)", textDecoration: "none" }}>
          ← All categories
        </Link>
      </div>
        ...
        <Form method="post" key={cat.id} style={{ display: "grid", gap: ".7rem" }}>
          ...
          <Field label="Parent">
            <select name="parentId" className={select} defaultValue={cat.parentId ?? ""}>
              ...
              {options
                .filter((o) => !blocked.includes(o.id))
                .map((o) => (
          ...
          <Field label="SEO title" hint="Blank = the category name.">
            <input name="seoTitle" defaultValue={cat.seoTitle} className={input} />
          </Field>
