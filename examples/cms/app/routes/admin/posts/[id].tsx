...
export async function loader({ request, params }: LoaderArgs) {
  ...
  return {
    post,
    categories: categoryTreeFlat(),
    media: listMedia(),
    galleryIds: getPostGalleryIds(post.id),
    customFields: loadEntityFields("post", post.id),
  };
}
...
export function ErrorBoundary({ error }: { error: unknown }) {
  ...
  return (
    <div className="admin-panel">
      ...
      <Link to="/admin/posts" style={{ color: "var(--accent)", fontWeight: 600 }}>
        ← Back to posts
      </Link>
    </div>
  );
      ...
      <div className="admin-bar">
        ...
        <div className="toolbar">
          {post.status === "published" ? (
            <Link to={`/posts/${post.slug}`} className={ghostButton}>
              View ↗
            </Link>
          ) : null}
          <Form
