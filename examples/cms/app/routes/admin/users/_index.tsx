...
import { requirePermission, type AdminUser } from "../../../auth.server.ts";
import { deleteUser, listUsers, type User } from "../../../models/users.server.ts";
import { userRoleNames, roleByName, directRoleMemberCount, userRoleIds } from "../../../models/rbac.server.ts";
...
export async function action({ request, formData }: ActionArgs): Promise<FormState | Response> {
  ...
  if (String(formData.get("intent")) === "delete") {
    ...
    if (admin && userRoleIds(id).includes(admin.id) && directRoleMemberCount(admin.id) <= 1) return flashFail({ error: "Can’t delete the last administrator." });
  ...
  return (
    <>
      <div className="admin-bar">
        ...
        <Link to="/admin/users/new" className={primaryButton}>New user</Link>
      </div>
      {state?.error ? <ErrorNote>{state.error}</ErrorNote> : null}
      <table className="admin-table">
        <thead><tr><th>User</th><th>Email (2FA)</th><th>Roles</th><th>Sign-in</th><th style={{ width: "1%" }}></th></tr></thead>
        <tbody>
          {users.map((u) => (
            <tr key={u.id}>
              <td>
                <Link to={`/admin/users/${u.id}`} style={{ fontWeight: 600, textDecoration: "none" }}>{u.displayName}</Link>
                {" "}<code style={{ color: "var(--muted)" }}>@{u.username}</code>
                {u.id === me ? <> <Badge tone="muted">you</Badge></> : null}
              </td>
              <td>{u.email ?? <span style={{ color: "var(--muted)" }}>—</span>}</td>
              <td>{u.roleNames.length ? <span style={{ display: "inline-flex", gap: ".3rem", flexWrap: "wrap" }}>{u.roleNames.map((n) => <Badge key={n} tone={n === "Administrator" ? "published" : "muted"}>{n}</Badge>)}</span> : <span style={{ color: "var(--muted)" }}>—</span>}</td>
              <td><Badge tone="muted">{u.provider}</Badge></td>
              <td>
                <div className="toolbar">
                  <Link to={`/admin/users/${u.id}`} className={ghostButton}>Edit</Link>
                  {u.id !== me ? (
