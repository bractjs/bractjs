...
import { listRoles, setUserRoles, type Role } from "../../../models/rbac.server.ts";
...
export async function action({ request, formData }: ActionArgs): Promise<FormState | Response> {
  await requirePermission(request, "users.manage");
  let data: UserInput;
  try { data = await validate<UserInput>(UserCreateSchema, formData); }
...
export function meta() { return [{ title: "New user | CMS Admin" }]; }
    ...
    <>
      ...
      <div className="admin-panel" style={{ maxWidth: "520px" }}>
        <Form method="post" style={{ display: "grid", gap: ".7rem" }}>
          <Field label="Username"><input name="username" required autoComplete="off" className={input} /></Field>
          {fe.username ? <ErrorNote>{fe.username[0]}</ErrorNote> : null}
          <Field label="Display name"><input name="displayName" className={input} /></Field>
          <Field label="Email" hint="The 2FA sign-in code is sent here."><input name="email" type="email" required autoComplete="off" className={input} /></Field>
            ...
            <div style={{ display: "grid", gap: ".3rem" }}>
              {roles.map((r) => (
                <label key={r.id} style={{ display: "flex", gap: ".5rem", alignItems: "center", fontSize: ".9rem" }}>
                  <input type="checkbox" name="roles" value={r.id} defaultChecked={r.name === "Editor"} /> {r.name}
                </label>
              ))}
            </div>
          </fieldset>
          <Field label="Password" hint="At least 6 characters."><input name="password" type="password" required autoComplete="new-password" className={input} /></Field>
          {fe.password ? <ErrorNote>{fe.password[0]}</ErrorNote> : null}
          {state?.error && !fe.username ? <ErrorNote>{state.error}</ErrorNote> : null}
          <div><button type="submit" className={primaryButton}>Create user</button></div>
        </Form>
      </div>
    </>
  );
