...
import { PERMISSION_GROUPS, PERMISSIONS } from "../../../permissions.ts";
import { Badge, ErrorNote, Field, input, primaryButton } from "../../../ui.tsx";
import { type NamedInput, RoleSchema } from "../../../validation.ts";
...
export function ErrorBoundary({ error }: { error: unknown }) {
  return (
    <div className="admin-panel">
      <h1 style={{ marginTop: 0 }}>Role not found</h1>
      <p style={{ color: "var(--admin-muted)" }}>{error instanceof Error ? error.message : "Error"}</p>
      <Link to="/admin/roles" style={{ color: "var(--admin-accent)", fontWeight: 600 }}>
        ← Back to roles
      </Link>
    </div>
  );
...
export function meta() {
  return [{ title: "Edit role | CMS Admin" }];
}
      ...
      <div className="admin-bar">
        <h1 style={{ margin: 0 }}>
          ...
          {role.isSystem ? (
            <>
              {" "}
              <Badge tone="published">system</Badge>
            </>
          ) : null}
        </h1>
        <Link to="/admin/roles" style={{ color: "var(--admin-accent)", textDecoration: "none" }}>
          ← All roles
        </Link>
      </div>
      <div className="admin-panel" style={{ maxWidth: "640px" }}>
        <Form method="post" key={role.id} style={{ display: "grid", gap: ".8rem" }}>
          ...
          <div>
            ...
            {role.isSystem ? (
              <p style={{ color: "var(--admin-muted)", fontSize: ".85rem", margin: ".3rem 0 0" }}>
