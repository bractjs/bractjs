          ...
          <table className="admin-table">
            ...
            <tbody>
              {roles.map((r) => (
                <tr key={r.id}>
                  <td>
                    <Link to={`/admin/roles/${r.id}`} style={{ fontWeight: 600, textDecoration: "none" }}>
                      {r.name}
                    </Link>
                    {r.isSystem ? (
                      <>
                    ...
                    {r.description ? (
                      <div style={{ color: "var(--admin-muted)", fontSize: ".8rem" }}>{r.description}</div>
                    ) : null}
                  </td>
                  <td>{r.isSystem ? "all" : r.perms}</td>
                  <td>
                    <div className="toolbar">
                      <Link to={`/admin/roles/${r.id}`} className={ghostButton}>
                        Edit
                      </Link>
                      {!r.isSystem ? (
