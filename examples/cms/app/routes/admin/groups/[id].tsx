...
import {
  getGroup, groupRoleIds, groupMemberIds, listRoles, setGroupMembers, setGroupRoles, updateGroup, type Group, type Role,
...
export async function loader({ request, params }: LoaderArgs): Promise<Data> {
  ...
  return { group, roles: listRoles(), users: listUsers(), roleIds: groupRoleIds(group.id), memberIds: groupMemberIds(group.id) };
}
...
export async function action({ request, params, formData }: ActionArgs): Promise<FormState | Response> {
  ...
  if (!group) throw new HttpError(404, "Group not found.");
  let data: NamedInput;
  try { data = await validate<NamedInput>(GroupSchema, formData); }
...
export function ErrorBoundary({ error }: { error: unknown }) {
  return <div className="admin-panel"><h1 style={{ marginTop: 0 }}>Group not found</h1><p style={{ color: "var(--admin-muted)" }}>{error instanceof Error ? error.message : "Error"}</p><Link to="/admin/groups" style={{ color: "var(--admin-accent)", fontWeight: 600 }}>← Back to groups</Link></div>;
}
export function meta() { return [{ title: "Edit group | CMS Admin" }]; }
  ...
  return (
    <>
      <div className="admin-bar">
        ...
        <Link to="/admin/groups" style={{ color: "var(--admin-accent)", textDecoration: "none" }}>← All groups</Link>
      </div>
      <div className="admin-panel" style={{ maxWidth: "720px" }}>
        <Form method="post" key={group.id} style={{ display: "grid", gap: ".9rem" }}>
          ...
          <Field label="Description"><input name="description" defaultValue={group.description} className={input} /></Field>
          <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "1fr 1fr" }}>
            <fieldset style={{ border: "1px solid var(--admin-line)", borderRadius: "8px", padding: ".6rem .8rem" }}>
              <legend style={{ fontSize: ".78rem", color: "var(--admin-muted)", textTransform: "uppercase" }}>Roles granted</legend>
            ...
            <fieldset style={{ border: "1px solid var(--admin-line)", borderRadius: "8px", padding: ".6rem .8rem" }}>
              <legend style={{ fontSize: ".78rem", color: "var(--admin-muted)", textTransform: "uppercase" }}>Members</legend>
              <div style={{ display: "grid", gap: ".3rem", maxHeight: "16rem", overflow: "auto" }}>
                {users.map((u) => (
                  <label key={u.id} style={{ display: "flex", gap: ".5rem", alignItems: "center", fontSize: ".9rem" }}>
                    <input type="checkbox" name="members" value={u.id} defaultChecked={memberIds.includes(u.id)} /> {u.displayName} <span style={{ color: "var(--admin-muted)" }}>@{u.username}</span>
                  </label>
                ))}
              </div>
            </fieldset>
          ...
          <div><button type="submit" className={primaryButton}>Save group</button></div>
        </Form>
      </div>
    </>
  );
