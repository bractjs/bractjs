...
import { createGroup, deleteGroup, groupMemberCount, groupRoleIds, listGroups, type Group } from "../../../models/rbac.server.ts";
...
import { EmptyState, ErrorNote, Field, ghostButton, dangerButton, input, primaryButton } from "../../../ui.tsx";
...
export async function loader({ request }: LoaderArgs): Promise<Data> {
  ...
  return { groups: listGroups().map((g) => ({ ...g, members: groupMemberCount(g.id), roles: groupRoleIds(g.id).length })) };
}

export async function action({ request, formData }: ActionArgs): Promise<FormState | Response> {
  await requirePermission(request, "roles.manage");
  if (String(formData.get("intent")) === "delete") { deleteGroup(String(formData.get("id") ?? "")); return flashRedirect("/admin/groups", "Group deleted"); }
  let data: NamedInput;
  ...
  return res.id ? flashRedirect(`/admin/groups/${res.id}`, "Group created") : flashFail({ error: res.reason });
}

export function meta() { return [{ title: "Groups | CMS Admin" }]; }
  ...
  return (
    <div style={{ display: "grid", gap: "1.2rem", gridTemplateColumns: "1fr 320px", alignItems: "start" }}>
      <div className="admin-panel">
        ...
        {groups.length === 0 ? <EmptyState>No groups yet — bundle roles and assign members to them.</EmptyState> : (
          <table className="admin-table">
            <thead><tr><th>Name</th><th>Roles</th><th>Members</th><th style={{ width: "1%" }}></th></tr></thead>
            <tbody>
              {groups.map((g) => (
                <tr key={g.id}>
                  <td><Link to={`/admin/groups/${g.id}`} style={{ fontWeight: 600, textDecoration: "none" }}>{g.name}</Link>{g.description ? <div style={{ color: "var(--admin-muted)", fontSize: ".8rem" }}>{g.description}</div> : null}</td>
                  <td>{g.roles}</td>
                  <td>{g.members}</td>
                  <td><div className="toolbar">
                    <Link to={`/admin/groups/${g.id}`} className={ghostButton}>Edit</Link>
                    <Form method="post" style={{ margin: 0 }}><input type="hidden" name="intent" value="delete" /><input type="hidden" name="id" value={g.id} /><button type="submit" className={dangerButton}>Delete</button></Form>
                  </div></td>
                </tr>
              ))}
            </tbody>
