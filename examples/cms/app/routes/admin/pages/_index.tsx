...
import { deletePage, pageTreeFlat, type PageNode } from "../../../models/pages.server.ts";
  ...
  return (
    <>
      <div className="admin-bar">
        ...
        <Link to="/admin/pages/new" className={primaryButton}>New page</Link>
      </div>
      {state?.error ? <ErrorNote>{state.error}</ErrorNote> : null}
      {pages.length === 0 ? (
        <EmptyState>No pages yet. <Link to="/admin/pages/new">Create one →</Link></EmptyState>
      ) : (
        <table className="admin-table">
          <thead>
            <tr><th>Title</th><th>Status</th><th>Order</th><th style={{ width: "1%" }}></th></tr>
          </thead>
          <tbody>
            {pages.map((p) => (
              <tr key={p.id}>
                <td>
                  ...
                  <Link to={`/admin/pages/${p.id}`} style={{ fontWeight: 600, textDecoration: "none" }}>{p.title}</Link>
                  <span style={{ color: "var(--muted)", fontSize: ".78rem", marginLeft: ".4rem" }}><code>/{p.slug}</code></span>
                </td>
                <td><Badge tone={p.status === "published" ? "published" : "draft"}>{p.status}</Badge></td>
                <td style={{ color: "var(--muted)" }}>{p.menuOrder}</td>
                <td>
                  <div className="toolbar">
                    <Link to={`/admin/pages/${p.id}`} className={ghostButton}>Edit</Link>
