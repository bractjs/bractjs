...
import {
  addMenuItem, getMenu, menuTree, reorderMenuItems, updateMenuSettings,
  type Menu, type MenuItemNode,
} from "../../../models/menus.server.ts";
import { categoryTreeFlat, type CategoryNode } from "../../../models/categories.server.ts";
import { pageTreeFlat, type PageNode } from "../../../models/pages.server.ts";
import { MenuItemSchema, type MenuItemInput } from "../../../validation.ts";
...
import { MenuTreeEditor, type EditorNode } from "../../../components/MenuTreeEditor.tsx";

const toEditor = (ns: MenuItemNode[]): EditorNode[] => ns.map((n) => ({
  id: n.id, label: n.label, cssClass: n.cssClass,
...
export async function action({ request, params, formData }: ActionArgs): Promise<FormState | Response> {
  ...
  if (intent === "reorder") {
    try {
      ...
      const nodes = (Array.isArray(raw) ? raw : []).map((n) => {
        ...
        return { id: String(o.id), parentId: o.parentId ? String(o.parentId) : null, position: Number(o.position) || 0, label: String(o.label ?? ""), cssClass: String(o.cssClass ?? "") };
      });
      reorderMenuItems(menu.id, nodes);
    } catch { return flashFail({ error: "Couldn’t save the menu layout." }); }
  ...
  try { data = await validate<MenuItemInput>(MenuItemSchema, formData); }
...
export function ErrorBoundary({ error }: { error: unknown }) {
  return <div className="admin-panel"><h1 style={{ marginTop: 0 }}>Menu not found</h1><p style={{ color: "var(--admin-muted)" }}>{error instanceof Error ? error.message : "Error"}</p><Link to="/admin/menus" style={{ color: "var(--admin-accent)", fontWeight: 600 }}>← Back to menus</Link></div>;
}

export function meta() { return [{ title: "Manage menu | CMS Admin" }]; }
    ...
    <>
      <div className="admin-bar">
        <div style={{ display: "flex", gap: ".6rem", alignItems: "baseline" }}>
          ...
          <span style={{ color: "var(--admin-muted)", textTransform: "capitalize" }}>{menu.location} menu</span>
        </div>
        <Link to="/admin/menus" style={{ color: "var(--admin-accent)", textDecoration: "none" }}>← All menus</Link>
      </div>
        ...
        <div style={{ display: "grid", gap: "1.2rem" }}>
          <div className="admin-panel">
            ...
            <Form method="post" style={{ display: "grid", gap: ".7rem" }}>
              ...
              <Field label="Page (if type = page)">
                <select name="pageId" className={select} defaultValue=""><option value="">—</option>{pages.map((p) => <option key={p.id} value={p.id}>{"  ".repeat(p.depth)}{p.title}</option>)}</select>
              </Field>
              <Field label="Category (if type = category)">
                <select name="categoryId" className={select} defaultValue=""><option value="">—</option>{categories.map((c) => <option key={c.id} value={c.id}>{"  ".repeat(c.depth)}{c.name}</option>)}</select>
              </Field>
              <Field label="URL (if type = custom)"><input name="url" placeholder="/contact or https://…" className={input} /></Field>
              {state?.error ? <ErrorNote>{state.error}</ErrorNote> : null}
              <div><button type="submit" className={primaryButton}>Add item</button></div>
            </Form>
          </div>
          ...
          <div className="admin-panel">
            ...
            <Form method="post" key={menu.id} style={{ display: "grid", gap: ".7rem" }}>
              ...
              <Field label="Main menu class" hint="On the root <ul>."><input name="menuClass" defaultValue={menu.menuClass} className={input} /></Field>
              <Field label="Sub-menu class" hint="On every nested <ul>."><input name="submenuClass" defaultValue={menu.submenuClass} className={input} /></Field>
              <Field label="Item class" hint="On every <li>; a per-item class adds to it."><input name="itemClass" defaultValue={menu.itemClass} className={input} /></Field>
              <div><button type="submit" className={ghostButton}>Save classes</button></div>
            </Form>
          </div>
        </div>
      </div>
