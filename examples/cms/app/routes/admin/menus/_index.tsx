...
import { createMenu, deleteMenu, listMenus, menuItems, type Menu } from "../../../models/menus.server.ts";
import { MenuSchema, type MenuInput } from "../../../validation.ts";
import { fromValidationError, type FormState } from "../../../form.ts";
import { flashFail, flashRedirect } from "../../../flash.server.ts";
import { dangerButton, EmptyState, ErrorNote, Field, ghostButton, input, primaryButton, select } from "../../../ui.tsx";
      ...
      <div style={{ display: "grid", gap: "1.2rem", maxWidth: "760px" }}>
          ...
          {usedLocations.size >= 2 ? (
            ...
            <Form method="post" style={{ display: "grid", gap: ".7rem" }}>
              ...
              <Field label="Location">
                <select name="location" className={select} defaultValue={usedLocations.has("header") ? "footer" : "header"}>
              ...
              <div><button type="submit" className={primaryButton}>Create menu</button></div>
            </Form>
          )}
        </div>
        ...
        {menus.length === 0 ? (
          ...
          <table className="admin-table">
            <thead><tr><th>Name</th><th>Location</th><th>Items</th><th style={{ width: "1%" }}></th></tr></thead>
            <tbody>
              {menus.map((m) => (
                <tr key={m.id}>
                  <td><Link to={`/admin/menus/${m.id}`} style={{ fontWeight: 600, textDecoration: "none" }}>{m.name}</Link></td>
                  <td style={{ textTransform: "capitalize" }}>{m.location}</td>
                  <td style={{ color: "var(--muted)" }}>{m.count}</td>
                  <td>
                    <div className="toolbar">
                      <Link to={`/admin/menus/${m.id}`} className={ghostButton}>Manage</Link>
