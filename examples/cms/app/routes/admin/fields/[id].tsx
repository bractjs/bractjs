...
export async function action({ request, params, formData }: ActionArgs): Promise<FormState | Response> {
  ...
  if (intent === "remove") {
    removeField(String(formData.get("fieldId") ?? ""));
    return flashRedirect(to, "Field removed");
  }
  if (intent === "up" || intent === "down") {
    moveField(String(formData.get("fieldId") ?? ""), intent);
    return flashRedirect(to, "Field reordered");
  }
  if (intent === "group") {
  ...
  let data: FieldInput;
  try {
    data = await validate<FieldInput>(FieldSchema, formData);
  } catch (err) {
    return flashFail(await fromValidationError(err));
  }
...
export function ErrorBoundary({ error }: { error: unknown }) {
  ...
  return (
    <div className="admin-panel">
      ...
      <Link to="/admin/fields" style={{ color: "var(--admin-accent)", fontWeight: 600 }}>
        ← Back to fields
      </Link>
    </div>
  );
...
export function meta() {
  return [{ title: "Edit field group | CMS Admin" }];
}
  ...
  return (
    <>
      <div className="admin-bar">
        ...
        <Link to="/admin/fields" style={{ color: "var(--admin-accent)", textDecoration: "none" }}>
          ← All groups
        </Link>
      </div>
      <div style={{ display: "grid", gap: "1.2rem", gridTemplateColumns: "1fr 340px", alignItems: "start" }}>
        <div className="admin-panel">
          ...
          {fields.length === 0 ? (
            <EmptyState>No fields yet — add one on the right.</EmptyState>
          ) : (
              ...
              {fields.map((f, i) => (
                ...
                >
                  <span style={{ flex: 1 }}>
                    ...
                    <code style={{ color: "var(--admin-muted)", marginLeft: ".4rem", fontSize: ".78rem" }}>
                      {f.name}
                    </code>
                  </span>
                  ...
                  <Form method="post" style={{ margin: 0 }}>
                    <input type="hidden" name="intent" value="up" />
                    <input type="hidden" name="fieldId" value={f.id} />
                    <button type="submit" disabled={i === 0} className={ghostButton} aria-label="Move up">
                      ↑
                    </button>
                  </Form>
                  <Form method="post" style={{ margin: 0 }}>
                    <input type="hidden" name="intent" value="down" />
                    <input type="hidden" name="fieldId" value={f.id} />
                    <button
                      type="submit"
                  ...
                  <Form method="post" style={{ margin: 0 }}>
                    ...
                    <input type="hidden" name="fieldId" value={f.id} />
                    <button type="submit" className={dangerButton} aria-label="Remove">
                      ✕
                    </button>
                  </Form>
                </li>
        ...
        <div style={{ display: "grid", gap: "1.2rem" }}>
          <div className="admin-panel">
            ...
            <Form method="post" style={{ display: "grid", gap: ".7rem" }}>
              ...
              <Field label="Key" hint="Blank = from label. Lowercase, letters/numbers/underscores.">
