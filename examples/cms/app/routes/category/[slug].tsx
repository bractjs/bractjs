...
import { categoryAncestors, getCategoryBySlug, type Category } from "../../models/categories.server.ts";
...
type Data = { category: Category; ancestors: Category[]; posts: PostWithRefs[]; header: ResolvedMenu; footer: ResolvedMenu };
...
export default function CategoryPage() {
  ...
  return (
    <SiteFrame header={header} footer={footer}>
      <Breadcrumb items={[{ label: "Home", to: "/" }, ...ancestors.map((a) => ({ label: a.name, to: `/category/${a.slug}` })), { label: category.name }]} />
      <h1 className="prose" style={{ marginTop: 0 }}>{category.name}</h1>
      {category.description ? <p style={{ color: "var(--muted)", marginTop: "-.5rem" }}>{category.description}</p> : null}
      {posts.length === 0 ? (
