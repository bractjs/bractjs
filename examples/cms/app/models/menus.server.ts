...
export type Menu = { id: string; name: string; location: MenuLocation; menuClass: string; submenuClass: string; itemClass: string; createdAt: number };
...
export function createMenu(input: { name: string; location: MenuLocation }): { ok: boolean; reason?: string; id?: string } {
  if (getMenuByLocation(input.location)) return { ok: false, reason: `A ${input.location} menu already exists.` };
  const id = newId();
  db.run("INSERT INTO menus (id, name, location, createdAt) VALUES (?,?,?,?)", [id, input.name, input.location, nowTs()]);
  return { ok: true, id };
}
...
export function menuItems(menuId: string): MenuItem[] {
  return db.query<MenuItem, [string]>("SELECT * FROM menu_items WHERE menuId = ? ORDER BY position ASC").all(menuId);
}

export function addMenuItem(
  menuId: string,
  input: { label: string; type: MenuItem["type"]; pageId: string | null; categoryId: string | null; url: string | null },
): MenuItem {
  // New items land at the end of the top level; nest/reorder happens in the tree editor.
  const max = db.query<{ m: number | null }, [string]>("SELECT MAX(position) AS m FROM menu_items WHERE menuId = ? AND parentId IS NULL").get(menuId)?.m;
  const id = newId();
  db.run(
...
export function updateMenuSettings(menuId: string, input: { menuClass: string; submenuClass: string; itemClass: string }): void {
  db.run("UPDATE menus SET menuClass = ?, submenuClass = ?, itemClass = ? WHERE id = ?", [input.menuClass, input.submenuClass, input.itemClass, menuId]);
}
...
export function menuTree(menuId: string): MenuItemNode[] {
  const flat = db.query<MenuItem, [string]>("SELECT * FROM menu_items WHERE menuId = ? ORDER BY position ASC").all(menuId);
...
export function reorderMenuItems(
  ...
  const owned = new Set(db.query<{ id: string }, [string]>("SELECT id FROM menu_items WHERE menuId = ?").all(menuId).map((r) => r.id));
  ...
  tx(() => {
    ...
    for (const n of nodes) {
      ...
      db.run("UPDATE menu_items SET parentId = ?, position = ?, label = ?, cssClass = ? WHERE id = ? AND menuId = ?", [
...
function hrefFor(it: MenuItem): string {
  ...
  if (it.type === "category") {
    const slug = db.query<{ slug: string }, [string]>("SELECT slug FROM categories WHERE id = ?").get(it.categoryId ?? "")?.slug;
...
export function resolvedMenu(location: MenuLocation): ResolvedMenu {
  ...
  const resolve = (nodes: MenuItemNode[]): MenuNode[] =>
    nodes.map((it) => ({ id: it.id, label: it.label, href: hrefFor(it), cssClass: it.cssClass, children: resolve(it.children) }));
  return { menuClass: menu.menuClass, submenuClass: menu.submenuClass, itemClass: menu.itemClass, items: resolve(menuTree(menu.id)) };
}
