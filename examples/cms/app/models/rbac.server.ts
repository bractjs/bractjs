...
export function directRoleMemberCount(roleId: string): number {
  return (
    db.query<{ n: number }, [string]>("SELECT COUNT(*) AS n FROM user_roles WHERE roleId = ?").get(roleId)
      ?.n ?? 0
  );
}
export function rolePermissions(roleId: string): Permission[] {
  return db
    .query<{ permission: string }, [string]>("SELECT permission FROM role_permissions WHERE roleId = ?")
    .all(roleId)
...
export function createRole(input: { name: string; description: string }): {
  ...
  if (db.query("SELECT 1 FROM roles WHERE name = ?").get(input.name))
    return { ok: false, reason: "A role with that name exists." };
  const id = newId();
  db.run("INSERT INTO roles (id, name, description, isSystem, createdAt) VALUES (?,?,?,0,?)", [
    id,
    input.name,
    input.description,
    nowTs(),
...
export function setRolePermissions(roleId: string, permissions: string[]): void {
  ...
  tx(() => {
    ...
    for (const p of permissions)
      db.run("INSERT OR IGNORE INTO role_permissions (roleId, permission) VALUES (?,?)", [roleId, p]);
  });
}
...
export function createGroup(input: { name: string; description: string }): {
  ...
  if (db.query("SELECT 1 FROM groups WHERE name = ?").get(input.name))
    return { ok: false, reason: "A group with that name exists." };
  const id = newId();
  db.run("INSERT INTO groups (id, name, description, createdAt) VALUES (?,?,?,?)", [
    id,
    input.name,
    input.description,
    nowTs(),
...
export function groupRoleIds(groupId: string): string[] {
  return db
    .query<{ roleId: string }, [string]>("SELECT roleId FROM group_roles WHERE groupId = ?")
    .all(groupId)
    .map((r) => r.roleId);
}
export function setGroupRoles(groupId: string, roleIds: string[]): void {
  tx(() => {
    ...
    for (const r of roleIds)
      db.run("INSERT OR IGNORE INTO group_roles (groupId, roleId) VALUES (?,?)", [groupId, r]);
  });
}
export function groupMemberIds(groupId: string): string[] {
  return db
    .query<{ userId: string }, [string]>("SELECT userId FROM user_groups WHERE groupId = ?")
    .all(groupId)
    .map((r) => r.userId);
}
export function setGroupMembers(groupId: string, userIds: string[]): void {
  tx(() => {
    ...
    for (const u of userIds)
      db.run("INSERT OR IGNORE INTO user_groups (userId, groupId) VALUES (?,?)", [u, groupId]);
  });
}
export function groupMemberCount(groupId: string): number {
  return (
    db.query<{ n: number }, [string]>("SELECT COUNT(*) AS n FROM user_groups WHERE groupId = ?").get(groupId)
      ?.n ?? 0
  );
}
...
export function userRoleIds(userId: string): string[] {
  return db
    .query<{ roleId: string }, [string]>("SELECT roleId FROM user_roles WHERE userId = ?")
    .all(userId)
    .map((r) => r.roleId);
}
export function setUserRoles(userId: string, roleIds: string[]): void {
  tx(() => {
    ...
    for (const r of roleIds)
      db.run("INSERT OR IGNORE INTO user_roles (userId, roleId) VALUES (?,?)", [userId, r]);
  });
}
export function userGroupIds(userId: string): string[] {
  return db
    .query<{ groupId: string }, [string]>("SELECT groupId FROM user_groups WHERE userId = ?")
    .all(userId)
    .map((r) => r.groupId);
}
export function setUserGroups(userId: string, groupIds: string[]): void {
  tx(() => {
    ...
    for (const g of groupIds)
      db.run("INSERT OR IGNORE INTO user_groups (userId, groupId) VALUES (?,?)", [userId, g]);
  });
}
...
export function groupMembers(groupId: string): User[] {
  return db
    .query<User, [string]>(
      `SELECT ${USER_PUBLIC} FROM users WHERE id IN (SELECT userId FROM user_groups WHERE groupId = ?) ORDER BY displayName`,
    )
    .all(groupId);
}
