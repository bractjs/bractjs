...
import type { PageNode, Page } from "../models/pages.server.ts";
...
export function PageForm({
  ...
  return (
    <Form method="post" key={page?.id ?? "new"} style={{ display: "grid", gap: "1.2rem", gridTemplateColumns: "minmax(0, 1fr) 280px", alignItems: "start" }}>
      ...
      <div style={{ display: "grid", gap: ".9rem" }}>
        <div className="admin-panel" style={{ display: "grid", gap: ".7rem" }}>
          ...
          <Field label="Menu order" hint="Lower numbers sort first.">
            <input name="menuOrder" type="number" defaultValue={String(page?.menuOrder ?? 0)} className={input} />
          </Field>
        ...
        <div className="admin-panel" style={{ display: "grid", gap: ".7rem" }}>
          ...
          <Field label="SEO title" hint="Blank = the page title."><input name="seoTitle" defaultValue={page?.seoTitle ?? ""} className={input} /></Field>
          {fe.seoTitle ? <ErrorNote>{fe.seoTitle[0]}</ErrorNote> : null}
          <Field label="Meta description" hint="~155 chars."><textarea name="seoDescription" defaultValue={page?.seoDescription ?? ""} className={`${textarea} min-h-20`} /></Field>
