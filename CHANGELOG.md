...
- **A no-JS form POST behind a TLS-terminating reverse proxy is no longer rejected with a 403.** The CSRF gate compares the browser's `Origin` against the request's own origin, and Bun rebuilds `request.url` from the forwarded `Host` on the plain-HTTP hop — so behind nginx/Caddy/Cloudflare the app read `http://app.example.com` while the browser truthfully sent `https://app.example.com`, the schemes disagreed, and the mutation was blocked. `X-Forwarded-Proto` and `X-Forwarded-Host` are now folded in as additional candidate origins. Only the no-JavaScript path was ever affected (a plain `<Form>` submitted before hydration or with JS off): every router-driven submit sets `X-BractJS-Action` and passes an earlier check, which is why this could ship unnoticed. The gate is not widened — browsers cannot set either header cross-origin without a CORS preflight the framework never approves, a client that _can_ forge headers could already pass via `X-BractJS-Action`, and `Sec-Fetch-Site` still vetoes cross-site requests first. nginx does not send these headers unless configured; the [deployment guide](docs/deployment.md#behind-a-reverse-proxy) now covers the setup.
...
- **ESLint (flat config) + Prettier replace Biome** as the repo's linter/formatter (`pnpm lint` / `pnpm format`; `pnpm format:check` in CI); `eslint-config-prettier` keeps the two from fighting. The linter is non-type-checked for speed, and the flat config reactivates the `eslint-disable` directives already present in the source.
...
- **CI seeds each example's `_generated/` before typechecking it.** The workspace typecheck ran against examples whose generated registries did not exist on a fresh clone, so CI failed on a tree that was actually fine. A `pretypecheck` script now runs `bractjs codegen:seed` per example.

### Docs

- CONTRIBUTING.md points at the full release guide (`PUBLISH_GUIDE.md`) instead of restating a partial version of it.
...
- **Biome** became the repo's linter/formatter, with `.editorconfig` populated (it was tracked but empty). (Replaced by ESLint + Prettier in 0.3.2.)
